(function(global){
    ...
    console.log("🌏 [Geography V8.5] 地理核心資料庫 (含圖像增強) 啟動...");
    ...
    const Utils = {
        ...
        // 智能圖像標籤生成器
        getImageQuery: (item) => {
            if (item.t[1] === "地形") return ``;
            if (item.t[1] === "氣候") return ``;
            if (item.t.includes("地圖") || item.t.includes("GIS")) return ``;
            if (item.t.includes("中國")) return ``;
            return ``;
        }
    };
    ...
    const geoDB = [
        ...
        { s:"地理", t:["國七","地形"], e:"海蝕平台", y:"海蝕崖", p:"波浪侵蝕", k:"海蝕洞", d:"海浪長期侵蝕海岸，形成懸崖與平台" },
        ...
        { s:"地理", t:["國七","地形"], e:"沙嘴", y:"沙嘴形成", p:"長岸流", k:"封閉海灣", d:"長岸流搬運沉積物形成沙嘴，封閉海灣形成潟湖" },
        ...
        { s:"地理", t:["國八","中國"], e:"青藏高原", y:"世界屋脊", p:"板塊抬升", k:"高寒", d:"海拔高、氣候寒冷，地形以高原為主" },
        ...
        { s:"地理", t:["國八","中國"], e:"雲貴高原", y:"石林", p:"溶蝕", k:"喀斯特地形", d:"石灰岩溶蝕形成奇特的石林與溶洞" },
        ...
        { s:"地理", t:["國九","世界"], e:"安地斯山脈", y:"高地氣候", p:"造山運動", k:"垂直分布", d:"高海拔地區呈現垂直氣候變化，發展特殊農業" },
        ...
        { s:"地理", t:["高三","永續"], e:"氣候變遷", y:"温室效應", p:"人為排放", k:"海平面上升", d:"溫室氣體增加導致全球暖化與海平面上升" },
    ...
    // ==========================================
    // 🏭 自動註冊工廠 (Auto-Registry Factory)
    // ==========================================
    geoDB.forEach((item, index) => {
        // 為了增加題數，每個項目我們生成兩種類型的題目：
        // Type 1: 特徵題 (Feature)
        // Type 2: 關鍵字題 (Keyword)
        
        const grade = item.t[0];
        const tags = ["geography", "地理", ...item.t];
        const pool = geoDB.filter(x => x.t[0] === grade); // 只取同年級做干擾項，確保難度一致
        
        // -------------------------------------
        // 1. 特徵題生成器 (Feature)
        // -------------------------------------
        const featId = `geo_${grade}_feat_${index}`;
        const featGen = function() {
            // 找干擾項：優先找同主題，若不足則找同年級其他
            const sameTopic = pool.filter(x => x.t[1] === item.t[1] && x.e !== item.e);
            const others = pool.filter(x => x.e !== item.e);
            
            let wrongs = [];
            if(sameTopic.length >= 3) wrongs = Utils.shuffle(sameTopic).slice(0,3).map(x=>x.y);
            else wrongs = Utils.shuffle(others).slice(0,3).map(x=>x.y);
            
            const opts = Utils.shuffle([item.y, ...wrongs]);
            const imgTag = Utils.getImageQuery(item);

            return {
                question: `【${item.t[1]}】關於「${item.e}」，下列敘述何者最為準確？`,
                options: opts,
        ...
        featGen.subject = "geography";
        featGen.tags = tags;
        
        // 存入避難所
        window.__GEOGRAPHY_REPO__[featId] = { func: featGen, tags: tags, subject: "geography" };

        // -------------------------------------
        // 2. 關鍵字題生成器 (Keyword)
        // -------------------------------------
        const keyId = `geo_${grade}_key_${index}`;
        const keyGen = function() {
            const others = pool.filter(x => x.e !== item.e);
            const wrongs = Utils.shuffle(others).slice(0, 3).map(x => x.k);
            const opts = Utils.shuffle([item.k, ...wrongs]);
            const imgTag = Utils.getImageQuery(item);

            return {
                question: `【${item.t[1]}】提到「${item.e}」，地理學上通常會聯想到哪個關鍵詞？`,
                options: opts,
        ...
        keyGen.subject = "geography";
        keyGen.tags = tags;

        window.__GEOGRAPHY_REPO__[keyId] = { func: keyGen, tags: tags, subject: "geography" };
    });

    console.log(`✅ [Geography] 已載入 ${geoDB.length * 2} 題型 (特徵+關鍵字) 至避難所。`);
