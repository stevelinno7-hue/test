(function(global){
    ...
    function init() {
        ...
        combinations.forEach(combo => {
            ...
            if (pool.length > 0) {
                // ------------------------------------------------------------
                // 🛠️ CRITICAL FIX: Tag Order Permutations
                // We register tags in BOTH orders to bypass the generator's strict sequence check.
                // Order 1: Subject, Topic, Grade (Matches request ['成語', '國七'])
                // Order 2: Subject, Grade, Topic (Matches previous logic)
                // ------------------------------------------------------------
                const registerTags = [
                    "chinese", 
                    topic, grade,          // Order A: Topic first (Fixes your current bug)
                    grade, topic,          // Order B: Grade first (Keeps backup compatibility)
                    "國文", "語文"
                ];
                ...
                G.registerTemplate(`chi_def_${grade}_${topic}`, (ctx, rnd) => {
                    ...
                    else if (topic === "古文" || topic === "唐詩") qText = `「${item.q}」出自何處或何人？`;
                ...
                G.registerTemplate(`chi_rev_${grade}_${topic}`, (ctx, rnd) => {
                    ...
                    let qText = "";
                    if (topic === "成語") qText = `下列哪個成語的意思是「${item.a}」？`;
                    else if (topic === "修辭") qText = `下列哪個句子使用了「${item.a}」？`;
        ...
        console.log(`🎉 國文題庫 (V7.2 Dual-Lock Fix) 已載入！註冊 ${combinations.size * 2} 個模板。`);
        
        // Debug Log to confirm tag structure
        if(combinations.size > 0) {
             const firstExample = Array.from(combinations)[0];
             const [g, t] = firstExample.split('_');
             console.log(`🔧 [Debug] V7.2 範例標籤結構: ["chinese", "${t}", "${g}", "${g}", "${t}", ...]`);
        }
    }
