(function(global){
    ...
    function init() {
        ...
        // 資料庫 (內容省略部分以節省空間，與之前相同，重點是下方的邏輯)
        const chiData = [
        ...
        combinations.forEach(combo => {
            ...
            if (pool.length > 0) {
                // ------------------------------------------------------------
                // 🛠️ CRITICAL FIX: Tag Order - Dual Registration
                // 我們同時註冊 "Topic First" 和 "Grade First" 兩種順序
                // 這樣無論 PaperGenerator 怎麼鎖定 (['成語', '國七'] 或 ['國七', '成語'])
                // 都一定能找到對應的模板。
                // ------------------------------------------------------------
                const registerTags = [
                    "chinese", 
                    topic, grade,          // 順序 A: 讓 ['成語', '國七'] 命中 (這就是您缺少的！)
                    grade, topic,          // 順序 B: 讓 ['國七', '成語'] 命中 (保留相容性)
                    "國文", "語文"
                ];
        ...
        // 這是最關鍵的一行，如果 Log 沒看到這行，代表檔案沒更新成功！
        console.log(`🎉 國文題庫 (V7.3 FINAL DEBUG) 已載入！註冊 ${combinations.size * 2} 個模板。`);
        
        if(combinations.size > 0) {
             const firstExample = Array.from(combinations)[0];
             const [g, t] = firstExample.split('_');
             console.log(`🔧 [Debug] V7.3 標籤檢查: ["chinese", "${t}", "${g}", "${g}", "${t}", ...]`);
        }
    }
