(function(global){
    ...
    function init() {
        ...
        // 4. 分數誘答生成器
        function generateFractionOptions(ansStr) {
            let [n, d] = ansStr.includes('/') ? ansStr.split('/').map(Number) : [Number(ansStr), 1];
            const opts = new Set([ansStr]);
            let safety = 0;
            while(opts.size < 4 && safety < 20) {
                const type = randInt(0, 3);
                let fakeStr = "";
                if (type === 0) fakeStr = toFrac(n + pick([1, -1, 2]), d);
                else if (type === 1) fakeStr = toFrac(n, d + pick([1, -1]));
                else if (type === 2 && n !== 0) fakeStr = toFrac(d, n);
                else fakeStr = toFrac(-n, d);
                if (fakeStr !== ansStr) opts.add(fakeStr);
                safety++;
            }
            ["1/2", "1", "0", "-1"].forEach(b => { if(opts.size < 4 && !opts.has(b)) opts.add(b); });
            return Array.from(opts);
        }

        // 5. 數值誘答生成器
        function generateNumericOptions(ans) {
            ...
            while(opts.size < 4 && safety < 20) {
                let fake;
                if (isInt) {
                    const offset = pick([1, -1, 10, -10, randInt(-5, 5)]);
                    fake = ans + offset;
                } else {
                    fake = fix(ans + pick([0.1, -0.1, 0.01, -0.01, 0.5]));
                }
        ...
        grades.forEach(grade => {
            ...
            pool.forEach((p, idx) => {
                ...
                G.registerTemplate(templateId, (ctx, rnd) => {
                    ...
                    if (p.isFrac) {
                        opts = shuffle(generateFractionOptions(ans)); 
                    } else if (p.type === 'text') {
                        const op = typeof p.opts === 'function' ? p.opts(vals) : p.opts;
                        opts = shuffle(op);
                    } else {
                        if (typeof ans === 'number' && !Number.isInteger(ans)) {
                            ans = fix(ans);
                        }
                        opts = shuffle(generateNumericOptions(ans));
                    }
        ...
        console.log(`✅ 數學題庫 (V6.0 百題旗艦版) 已載入完成，共 ${mathDB.length} 題。`);
    }
